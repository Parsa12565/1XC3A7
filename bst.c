...
int *array_of_sorted_keys(bstNode *node)
{
  ...
  int *Larray=array_of_sorted_keys(node->left_child);
  int *Rarray=array_of_sorted_keys(node->right_child);
  int i;
  for(i=0;i<num_nodes(node->left_child);i++)
  {
    array[i]=Larray[i];
  }
  array[i]=node->key;
  for(int j=0;j<num_nodes(node->right_child);j++)
  {
    i++;
    array[i]=Rarray[j];
  }
...
bstNode *balanced_tree_from_sorted_array(int *sorted_array, int length)
{
  ...
  bool was_inserted=false;
  tree=insert(tree,sorted_array[length/2-1+length%2],&was_inserted);
  int *Larray=malloc(((length-1)/2)*sizeof(int));
  int i;
  for(i=0;i<(length-1)/2;i++)
  {
    Larray[i]=sorted_array[i];
  }
  int *Rarray=malloc((length/2)*sizeof(int));
  for(int j=0;j<length/2;j++)
  {
    i++;
    Rarray[j]=sorted_array[i];
  }
  tree->left_child=balanced_tree_from_sorted_array(Larray,(length-1)/2);
  tree->right_child=balanced_tree_from_sorted_array(Rarray,length/2);
  free(Larray);
  free(Rarray);
  return tree;
}
...
bstNode *balanced_tree_copy(bstNode *tree)
{
  return balanced_tree_from_sorted_array(array_of_sorted_keys(tree), num_nodes(tree));
}
